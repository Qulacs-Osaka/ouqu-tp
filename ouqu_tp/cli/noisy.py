...
def getval_noisy_call(
        input_qasm_file: str = typer.Option(...), input_openfermion_file: str = typer.Option(...),
        p1: float = 0, p2: float = 0, pm: float = 0, pp: float = 0) -> None:
...
def sampleval_noisy_call(
    input_qasm_file: str = typer.Option(...), input_openfermion_file: str = typer.Option(...), shots: int = typer.Option(...),
        p1: float = 0, p2: float = 0, pm: float = 0, pp: float = 0) -> None:
    cpl_qasm: List[str] = subprocess.check_output(
        ['staq', '-m', '--evaluate-all', input_qasm_file]).decode().splitlines()
